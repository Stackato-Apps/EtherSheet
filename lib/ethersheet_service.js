...
var EtherSheetService = module.exports = function(config){
  ...
  if (dbUrl) {
    es.config.db_host = dbUrl.hostname;
    es.config.db_port = dbUrl.port;
    var path = dbUrl.path.slice(1).split(/[?#]/);
    es.config.db_name = path[0];
    var auth = dbUrl.auth.split(':');
    es.config.db_user = auth[0];
    es.config.db_password = auth[1];
  }
