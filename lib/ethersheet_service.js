...
var EtherSheetService = module.exports = function(config){
  ...
  /* for Cloud Foundry */
  es.config.host = process.env.VCAP_APP_HOST || es.config.host || localhost;
  es.config.port = process.env.VCAP_APP_PORT || es.config.port || 8000;
  ...
  if (vcapServices) {
    ...
    var credentials = vcapServices[serviceName][0]['credentials'];
    es.config.db_host = credentials['host'];
    es.config.db_port = credentials['port'];
    es.config.db_name = credentials['database'];
    es.config.db_user = credentials['user'];
    es.config.db_password = credentials['password'];
  }
